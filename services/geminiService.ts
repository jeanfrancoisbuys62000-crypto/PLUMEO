...
export const extractTextFromImage = async (base64Data: string, mimeType: string): Promise<string> => {
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      temperature: 0.1, // Basse température pour plus de fidélité à l'image
    }
  });
...
export const analyzeRedaction = async (text: string, consigne: Consigne | null): Promise<AnalysisResult> => {
  ...
  const systemInstruction = `
    ...
    Exemple: <error type="grammar" hint="Accord sujet-verbe" guidance="Quel est le sujet de 'mange' ?">Ils mange</error> une pomme.
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      ...
      responseSchema: {
        type: Type.OBJECT,
        properties: {
          summary: { type: Type.STRING, description: "Un résumé encourageant de 2-3 phrases." },
          score: { type: Type.NUMBER, description: "Une note indicative sur 40 (critères Brevet)." },
          ...
          annotatedText: { type: Type.STRING, description: "Le texte avec les balises <error>." }
        },
